...
public class QuestionnaireResponseValidatorR5Test  {
	...
	public void testAnswerWithCorrectType() {
		...
		when(myValSupport.validateCode(any(), any(), any(), any(), any(), nullable(String.class), any()))
	...
	public void testCodedAnswer() {
		...
		when(myValSupport.validateCode(any(), any(), eq("http://codesystems.com/system"), eq("code0"), any(), nullable(String.class), any()))
			.thenReturn(new IValidationSupport.CodeValidationResult().setCode("code0"));
		when(myValSupport.validateCode(any(), any(), eq("http://codesystems.com/system"), eq("code1"), any(), nullable(String.class), any()))
			.thenReturn(new IValidationSupport.CodeValidationResult().setSeverity(IValidationSupport.IssueSeverity.ERROR).setMessage("Unknown code"));
	...
	public void testEmbeddedItemInChoice() {
		...
		when(myValSupport.fetchValueSet(eq(valueSetRef)))
			.thenReturn(options);
		when(myValSupport.validateCode(any(), any(), eq(codeSystemUrl), eq(codeValue), any(String.class), anyString(), any()))
